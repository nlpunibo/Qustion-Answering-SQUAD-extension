...
def main():
    ...
    parser.add_argument("path_to_dataset_file", help="Path to the classifier dataset file", type=str)
    ...
    train.to_csv(str(data_path / "dataset_classifier_train.csv"), encoding='utf-8')
    val.to_csv(str(data_path / "dataset_classifier_val.csv"), encoding='utf-8')

    train_data = load_dataset('csv', data_files=str(data_path / "dataset_classifier_train.csv"))
    val_data = load_dataset('csv', data_files=str(data_path / "dataset_classifier_val.csv"))
    ...
    train_tokenized_datasets = train_data.map(squad.prepare_train_features_classifier,
                                              remove_columns=train_data['train'].column_names)
    val_tokenized_datasets = val_data.map(squad.prepare_train_features_classifier, remove_columns=val_data['train'].column_names)

    # Now that our data is ready for training, we can download the pretrained model and fine-tune it.
    model = DistilBertClassifier.from_pretrained(model_checkpoint)
    ...
    trainer.save_model(str(data_path / "classifier_model"))
