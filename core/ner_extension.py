...
def ner(test, path_to_ner_weights):
    ...
    squad = SQUAD(tokenizer, pad_on_right, max_length, doc_stride, squad_v2)
    ...
    # Get the weights needed to improve the answer selection
    with open(path_to_ner_weights) as json_file:
        ner_weights = json.load(json_file)

    # To get the final predictions we can apply our post-processing function to our raw predictions
    final_predictions = dict(squad.postprocess_qa_predictions_ner(test_data['train'], test_features, pred.predictions, ner_weights))
