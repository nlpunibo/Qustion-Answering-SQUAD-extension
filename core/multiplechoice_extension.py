...
def multiple_choice(test):
    ...
    # As multiple choice model_checkpoint we use our multiplechoice model
    multiple_choice_checkpoint = "nlpunibo/multiplechoice"
    multiple_choice_tokenizer = AutoTokenizer.from_pretrained(multiple_choice_checkpoint)
    ...
    squad = SQUAD(qa_tokenizer, pad_on_right, max_length, doc_stride, squad_v2, multiple_choice_tokenizer, device)
    ...
    # We can download the pretrained multiplechoice model.
    multiple_choice = DistilBertForMultipleChoice.from_pretrained(multiple_choice_checkpoint)

    # Tell pytorch to run this model on the GPU.
    if torch.cuda.is_available():
        multiple_choice.cuda()
    ...
    final_predictions = dict(squad.postprocess_qa_predictions_multiplechoice(test_data['train'], test_features, pred.predictions, multiple_choice))
